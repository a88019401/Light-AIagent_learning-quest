...
import BadgesView from "./components/BadgesView";
 // src/App.tsx 
 import SRLCycle from "./components/SRLCycle";
...
/* -----------------------------
   類型（僅供本檔使用）
------------------------------ */
type Tab = "learn" | "challenge" | "badges" | "srl";
...
function LockedTab({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative inline-block">
      {children}
      {/* 遮罩：擋住點擊，順便做灰霧效果 */}
      <div className="absolute inset-0 rounded-xl bg-white/70 backdrop-blur-[1px] 
                      flex items-center justify-center text-xs text-neutral-700 
                      font-medium cursor-not-allowed select-none z-10">
        尚未開放
      </div>
    </div>
  );
...
export default function App() {
  // 頁籤 / 視圖狀態
  const [tab, setTab] = useState<Tab>("srl");
